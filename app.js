  ...
  for (const [type, items] of Object.entries(results)) {
    ...
    for (const item of items) {
      ...
      html += `<div class="ioc-item">
        <span class="ioc-value copyable" onclick="copyText('${item.value.replace(/'/g,"\\'")}','${type}')">${item.value}</span>
        ${priv}${dga}${algo}
...
function renderFileResults(r) {
  const sevColor = { critical:'var(--red)', high:'var(--orange)', medium:'var(--yellow)', low:'var(--accent)' };
          ...
          <div class="card-body">
            ${Object.entries(r.hashes).map(([algo,hash])=>`
              <div class="hash-row">
                ...
                <div class="hash-val copyable" onclick="copyText('${hash}','${algo}')">${hash}</div>
              </div>`).join('')}
          </div>
        </div>
      ...
      <div class="flex-col" style="gap:16px">
        ...
        <div class="card">
          <div class="card-header">
            ...
            <button class="btn btn-ghost btn-sm" onclick="copyText(State.lastFileResult.strings.sample.join('\\n'),'Strings')">Copy</button>
          </div>
          <div class="card-body">
...
function exportNavigatorLayer() {
  ...
  const layer = {
    ...
    techniques: [...State.selectedTTPs].map(id => ({
      techniqueID: id,
      tactic: ATTCK_DATA.techniques.find(t=>t.id===id)?.tactic,
      color: '#00d4aa',
      comment: '',
      enabled: true,
      score: 1,
...
async function refreshDatabase(query) {
  ...
  tbody.innerHTML = iocs.map(ioc => `
    <tr>
      <td class="mono copyable" onclick="copyText('${ioc.value.replace(/'/g,"\\'")}','IOC')" title="Click to copy">${escapeHtml(ioc.value)}</td>
